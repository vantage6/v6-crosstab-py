...
image:
	@echo "Building ${REGISTRY}/algorithm/crosstab:${TAG}-v6-${VANTAGE6_VERSION}"
	@echo "Building ${REGISTRY}/algorithm/crosstab:latest"
	docker buildx build \
		--tag ${REGISTRY}/algorithm/crosstab:${TAG}-v6-${VANTAGE6_VERSION} \
		--tag ${REGISTRY}/algorithm/crosstab:latest \
		--platform ${PLATFORMS} \
		-f ./Dockerfile \
